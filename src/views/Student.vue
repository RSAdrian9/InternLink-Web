<template>
  <v-container>
    <h1 class="text-h4 mb-4">Gestión de Estudiantes</h1>
    ...
    <v-dialog v-model="dialogDelete" max-width="500px">
      <v-card>
        <v-card-title class="text-h5">¿Estás seguro de que quieres eliminar a este estudiante?</v-card-title>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialogForm" max-width="600px">
        <v-card>
            <v-card-title class="text-h5">{{ formTitle }} Estudiante</v-card-title>
            <v-card-text>
              <v-form ref="userFormRef">
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12" sm="6">
                      <v-select
                        v-model="editedUser.degree"
                        :items="degreeOptions" label="Grado/Carrera *"
                        :rules="[rules.required]"
                        required
                        clearable
                      ></v-select>
...
import type { User, CreateUserDTO, UpdateUserDTO, Degree } from '../types/User';
...
const degreeOptions: Degree[] = ['1ºSMR', '2ºSMR', '1ºDAM', '2ºDAM', '2ºDAW'];
...
const headers = ref([
  ...
  { title: 'Grado/Carrera', key: 'degree' },
...
const editedUser = ref<User | CreateUserDTO>(JSON.parse(JSON.stringify(defaultStudentUser)));

const formTitle = computed(() => 'id' in editedUser.value ? 'Editar' : 'Añadir');

// Reglas de validación (igual que antes)
const rules = {
...
const addStudent = () => {
  editedUser.value = JSON.parse(JSON.stringify(defaultStudentUser));
  ...
  if (editedUser.value) {
    try {
      if ('id' in editedUser.value) {
        ...
        } as CreateUserDTO & { password_confirmation: string };
